...
from misc.visualization import draw_points_pushups, draw_points_situps, draw_points_chinups, draw_skeleton, draw_points_and_skeleton, joints_dict
...
def main(camera_id, filename, hrnet_m, hrnet_c, hrnet_j, hrnet_weights, hrnet_joints_set, image_resolution,
    ...
    while True:
        ...
        colorr = (0, 0, 0)
        thicknessr = -1
        frame = cv2.rectangle(frame, start_point, end_point, colorr, thicknessr)
        ...
        if exercise_type == 1:#for pushUps
            ...
            for i, (pt, pid) in enumerate(zip(pts, person_ids)):
                frame,angle = draw_points_and_skeleton(frame, pt, joints_dict()[hrnet_joints_set]['skeleton'], person_index=pid,
            ...
            color = (255, 255, 255)
            thickness = 2
            frame = cv2.putText(frame, str(angle), org, font,  
            ...
            for i, (pt, pid) in enumerate(zip(pts, person_ids)):
                frame,angle = draw_points_and_skeleton(frame, pt, joints_dict()[hrnet_joints_set]['skeleton'], person_index=pid,
                                             points_color_palette='gist_rainbow', skeleton_color_palette='jet',
                                             points_palette_samples=10,exercise_type=2)
            ...
            for i, (pt, pid) in enumerate(zip(pts, person_ids)):
                frame,dist = draw_points_and_skeleton(frame, pt, joints_dict()[hrnet_joints_set]['skeleton'], person_index=pid,
                                             points_color_palette='gist_rainbow', skeleton_color_palette='jet',
                                             points_palette_samples=10,exercise_type=3)
            ...
            color = (255, 255, 255)
            thickness = 2
            frame = cv2.putText(frame, str(dist), org, font,  
            fontScale, color, thickness, cv2.LINE_AA)
            if(len(pts)>0):
        ...
        if save_video:
            if video_writer is None:
                ...
                video_writer = cv2.VideoWriter('arnleft.avi', fourcc, video_framerate, (frame.shape[1], frame.shape[0]))
...
if __name__ == '__main__':
    ...
    parser.add_argument("--exercise_type", "-et", help="1 foor pushups,2 for situps, 3 for chinups", type=int, default=1)
